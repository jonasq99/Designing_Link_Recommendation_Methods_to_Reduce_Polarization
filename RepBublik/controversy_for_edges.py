...
from utils import (
    get_bad_and_good_nodes,
    get_centralities,
    load_bubble_diameter,
    sort_edges,
)
...
def get_tok_nodes(G, node_id_matrix, nodes, perc_k=5):
    ...
    red_topk = [
        node_id_matrix[i]
        for i, j in sorted(red_degrees.items(), key=lambda x: x[1], reverse=True)
    ][:k]
...
def compute_controversy_score(
    A, node_id_matrix, topic_obj, red_topk, blue_topk, perc=10, alpha=0.95
):
    """ """
    r_red_final = color_chain(
        csc_matrix(A), node_id_matrix, topic_obj.red_nodes, alpha=alpha
    )
    r_blue_final = color_chain(
        csc_matrix(A), node_id_matrix, topic_obj.blue_nodes, alpha=alpha
    )
...
def update_rwc(
    chunk, RWC, A, node_id_matrix, topic_obj, red_topk, blue_topk, perc=20, alpha=0.95
):
    ...
    for u, v in chunk:
        ...
        mat[v, u] = new_prob
        new = compute_controversy_score(
            mat, node_id_matrix, topic_obj, red_topk, blue_topk, perc, alpha
        )
...
parser = argparse.ArgumentParser(
    description="Run the entire/partial pipeline of FairRandomWalks experiments."
)
parser.add_argument(
    "-proc",
    type=str,
...
parser.add_argument(
    "-unweighted", type=str, default="false", help="Weighted or unweighted graph"
)
...
labels = np.array(
    [
        j
        for i, j in sorted(
            [(i, j) for i, j in id_matrix_node.items()], key=lambda x: x[0]
        )
    ]
)
...
RWC = compute_controversy_score(
    A, node_id_matrix, politics, red_topk, blue_topk, perc=10, alpha=0.95
)


candidate_edges = get_candidate_edges(
    G, politics.red_nodes, politics.blue_nodes, node_id_matrix, perc_k=10
)  # [:10]
candidate_edges = [
    (i, j)
    for i, j in candidate_edges
    if (id_matrix_node[i], id_matrix_node[j]) not in G.edges()
]
