...
class LoadData(object):
    ...
    def __init__(
        self, topic, uniform=True, edges_file="clickstream_weighted_edges.tsv"
    ):

        self.folder = "datasets/RePBubLik/{}/".format(topic)
        self.uniform = uniform
        self.blue_nodes = pd.read_csv(
            self.folder + topic + "_blue_pre_labeled_nodes.tsv", sep="\t", header=None
        )
        self.red_nodes = pd.read_csv(
            self.folder + topic + "_red_pre_labeled_nodes.tsv", sep="\t", header=None
        )
    ...
    def _different_weighted_neighbors(self):
        ...
        for n in self.G.nodes():
            ...
            if len(neighbors) != 0:
                ...
                dictionary_weights[n]["neigh"] = [n] + [
                    i[0] for i in self.G.in_edges(n)
                ]
                ...
                weights = [
                    self.G[i][n]["weight"] if i != n else 10
                    for i in dictionary_weights[n]["neigh"]
                ]
                normalized_weights = np.array(weights) / np.sum(weights)
                dictionary_weights[n]["weighted"] = normalized_weights
    ...
    def _weighted_edges(self, edges_file):
        ...
        print(df_edges)
        edges_multi = zip(
            list(df_edges[0].values), list(df_edges[1].values), list(df_edges[2].values)
        )

        edges = list(
            filter(
                lambda x: (x[0] in self.id_name) and (x[1] in self.id_name), edges_multi
            )
        )
    ...
    def _mapping_id_name(self):
        ...
        id_name_blue = {
            i: j
            for i, j in zip(
                list(self.blue_nodes[1].values), list(self.blue_nodes[0].values)
            )
            if i in id_nodes
        }
        id_name_red = {
            i: j
            for i, j in zip(
                list(self.red_nodes[1].values), list(self.red_nodes[0].values)
            )
            if i in id_nodes
        }
